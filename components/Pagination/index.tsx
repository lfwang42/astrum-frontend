...
export const Pagination: React.FC<PaginationProps> = ({
  ...
  const [disableNext, toggleNext] = useState<boolean>(
    rows?.length >= pageSize ? false : true
  );
  ...
  return (
    <div className="relative justify-center items-center">
      <div className="flex justify-center">
        <button className="" onClick={onPrevPage} disabled={disablePrev}>
          {"<<"}
        </button>
        <span>{currentPage}</span>
        <button className="" onClick={onNextPage} disabled={disableNext}>
          {">>"}
        </button>
      </div>
      <div className="absolute right-0 bottom-0 mr-2">
        {tableSizeLoading ? `---` : `${from}-${to}`} of{" "}
        {tableSizeLoading ? `???` : tableSize}
      </div>
    </div>
  );
};
