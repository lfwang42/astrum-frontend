...
//mostly based off how akasha
export const UpdatedDate: React.FC<UpdatedDateProps> = ({
    lastUpdate
}) => {
  ...
  return (
    <span className="text-slate-200 text-base">
      <span>{timestamp.value ? 'Refreshed ' : ''}</span><span>{timestamp.prefix ? timestamp.prefix : <></>}</span> <span>{timestamp.value ? timestamp.value : null}</span>
