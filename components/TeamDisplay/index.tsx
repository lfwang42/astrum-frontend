...
export const TeamDisplay: React.FC<TeamProps> = ({ team, short }) => {
  ...
  return (
    <div className="flex justify-start w-300 whitespace-nowrap gap-1 text-sm mr-1">
      {arr ? (
        arr.map((teammate, index) => {
          if (teammate.avatar === "any" || teammate.avatar == "none") {
            return (
              <div
                key={teammate.avatar + index}
                className={`bg-gray-700 p-[2px] rounded-md bg-opacity-75 flex justify-center items-center align-middle border-gray-400 border-2`}
              >
                <Image
                  width="25"
                  height="25"
            ...
            return (
              <div
                key={teammate.avatar + index}
                className={` ${colors[teammate.element!].back} ${
                  colors[teammate.element!].border
                } p-[2px] rounded-md relative items-center align-middle flex bg-opacity-75 border-2`}
              >
                <Image
                  width="25"
                  height="25"
                ...
                {teammate.fromUser ? (
                  <Image
                    width={17}
                    height={17}
                    className="text-gray-100 absolute right-[-5px] bottom-[-5px] border-gray-100 border-2 rounded-md bg-gray-600"
                    src={avatar}
