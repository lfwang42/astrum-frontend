    ...
    <div className="flex justify-start px-3 gap-2 m-2">
        {user ? (user.ranks.map((result) => {
            ...
            <Link key={`result-${user.uid}-${result.avatar_id}`} href={`/leaderboard/${best.calc_id}/`}>
              <div  className="justify-center text-center bg-slate-700 p-2 hover:bg-slate-400 max-w-28 min-h-36 hover:border-sky-900 border-2 border-none">
              <div className="flex align-middle items-center min-h-10"><span className="text-center text-sm mb-1 items-center align-middle font-medium text-wrap">{result.score_name}</span></div>
