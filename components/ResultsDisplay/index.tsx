    ...
    <div className="flex justify-start px-2 gap-2 m-2">
        {user ? (user.ranks.map((result) => {
            ...
            <Link key={`result-${user.uid}-${result.avatar_id}`} href={`/leaderboard/${best.calc_id}/`}>
              ...
              <div className="flex align-middle items-center justify-center min-h-8"><span className="text-center text-xs items-center align-middle font-medium text-wrap">{result.score_name}</span></div>
              <div className="flex justify-center gap-[2px] mb-[1px]">
                  ...
                  <Image src={`/icon/${best.tid}.png`} className="ml-1" height={30} width={30} alt="lightcone image" unoptimized />
              </div>  
